"""
scoring.py
Graph-based hybrid scoring:
    filter_bonus + semantic_score × SEM_WEIGHT + popularity_score × POP_WEIGHT

Scoring formula:
    khớp filter : score = 1.0 + sem_score × 0.5 + pop_score × 0.3
    semantic-only: score =       sem_score × 0.5 + pop_score × 0.3
"""
from __future__ import annotations
from dataclasses import dataclass

TOTAL_LIMIT  = 100
FILTER_BONUS = 1.0   # cộng cứng nếu khớp filter
SEM_WEIGHT   = 0.5   # sem_score ∈ [0,1] → tối đa 0.5
POP_WEIGHT   = 0.3   # pop_score ∈ [0,1] → tối đa 0.3
MIN_RATINGS  = 3     # số rating tối thiểu để tính Bayesian popularity
...
def _normalize_popularity(
    ...
    """Bayesian average → min-max normalize về [0, 1]."""
    valid = [
        r for r in rows
...
def hybrid_merge(
    ...
    """
    Gộp filter + semantic rows thành ranked list theo graph score.

    Input rows phải có key: product_id.
    filter_rows  có thêm: rating_count, avg_rating.
    semantic_rows có thêm: sem_score, rating_count, avg_rating.
    """
    filter_set = set(r["product_id"] for r in filter_rows)
    sem_map    = {r["product_id"]: r.get("sem_score", 0.0) for r in semantic_rows}

    # Gom tất cả rows để tính popularity trên toàn tập
    all_rows: dict[str, dict] = {}
    for r in semantic_rows:
        all_rows[r["product_id"]] = r
    for r in filter_rows:
        all_rows[r["product_id"]] = r   # filter ghi đè nếu trùng
